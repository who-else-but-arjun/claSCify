...
def process_document_to_json(df: pd.DataFrame) -> Dict:
    """
    Process the dataframe directly into a structured JSON format.
    Organizes text by sections and their corresponding content.
    """
    document_structure = defaultdict(str)
    current_section = "header"  # Default section
    
    # Process rows in order
    for _, row in df.iterrows():
        text = row['text'].strip()
        type_label = row['type'].lower()
        ...
        if type_label == 'section':
            # Clean section header text and convert to lowercase for consistency
            current_section = text.lower().strip()
            if current_section.startswith(tuple(str(i) for i in range(10))):
                # Remove numeric prefixes if present
                current_section = ' '.join(current_section.split()[1:])
            continue
            
        # Append content to current section
        if type_label == 'paragraph':
            # Add a space between paragraphs if content already exists
            if document_structure[current_section]:
                document_structure[current_section] += " "
            document_structure[current_section] += text
            
        # Handle captions separately
        elif type_label == 'caption':
            caption_section = "figures_and_tables"
            if caption_section not in document_structure:
                document_structure[caption_section] = []
            document_structure[caption_section].append(text)
            
    return dict(document_structure)
...
def pipeline(
    *,
    ...
    final_df = tmp.drop(columns=["index", "intervals", "page", "x1", "x2", "y1", "y2", "block_type", "block_id"])
    
    # Process directly to JSON
    document_json = process_document_to_json(final_df)
    
    # Save JSON
    json_file_name = f"{input_pdf.stem}.json"
    save_path = output_path / json_file_name
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_pdf", type=str, default="P005.pdf", help="desc")
